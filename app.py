...
CURRENCY_API_KEY = "cur_live_fu5Qwj51XDciBICSWKky2Zgjzf3DSa52yj6pqj4C"
...
with col1:
    from_currency = st.selectbox(
        "From Currency", currency_keys, index=st.session_state.from_idx
    )
...
with col3:
    to_currency = st.selectbox(
        "To Currency", currency_keys, index=st.session_state.to_idx
    )
...
if amount > 0 and from_c != to_c:
    try:
        result, rate, updated = convert_currency(amount, from_c, to_c)
...
with b1: st.button("1W", on_click=set_range, args=(7,))
with b2: st.button("1M", on_click=set_range, args=(30,))
with b3: st.button("3M", on_click=set_range, args=(90,))
with b4: st.button("1Y", on_click=set_range, args=(365,))
with b5: st.button("2Y", on_click=set_range, args=(730,))
with b6: st.button("5Y", on_click=set_range, args=(1825,))
with b7: st.button("10Y", on_click=set_range, args=(3650,))
...
if not df.empty:
    ...
    st.caption(
        "📌 Daily mid-market FX rates from CurrencyAPI.net. "
        "Ranges are calendar-accurate (XE-style)."
    )
else:
